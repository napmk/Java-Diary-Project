...
public class WinDeleteDiary extends JDialog {
	...
	public static void main(String[] args) {
		try {
			WinDeleteDiary dialog = new WinDeleteDiary("test","안녕하세요","");
	 ...
	 * Create the dialog.
	 */
	public WinDeleteDiary(String strID, String strName, String strDate) {//생성자 변경(인수 0개-> 2개로)
		sID = strID;
		sName = strName;
		sDate = strDate;
				...
				okButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent e) {
						try {
							...
							String sql = "delete from diaryTBL where id='";
							sql = sql + sID + "' and mDate='" + sDate + "'";
							//System.out.println(sql);
							if(stmt.executeUpdate(sql) > 0)
								setVisible(false);
								//JOptionPane.showMessageDialog(null, "정상 삭제 완료");
							else
